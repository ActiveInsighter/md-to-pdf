...
前端依赖由 `frontend/package-lock.json` 固定：
...
运行 smoke test 前需要提供对应的 Supabase 和 GitHub Actions 环境变量。测试会创建临时用户，并先创建一个未启动任务来验证上传后取消、数据库终态、Storage 对象清理、重复取消幂等和禁止重新启动；随后再创建独立任务，验证上传、调度、PDF 文件头、签名下载与最终清理流程。
