...
`public.pdf_jobs` 保存任务所有权、源文件名、文档名、文件路径、主题、选项、GitHub 运行信息、错误摘要和过期时间。
...
尚未进入队列的 `created/uploaded` 任务可以通过 `cancel-pdf-job` 取消。接口会先使用带状态条件的更新抢占任务，再删除该任务的输入对象，避免与并发启动操作发生破坏性竞态。已经排队或开始构建的任务不会被该接口取消。
...
迁移负责创建任务表、约束、索引、RLS、私有 Bucket、Storage Policy，并把 `pdf_jobs` 加入 Realtime publication。
...
当前实现按数据库中的 `expires_at` 清理普通任务：

- 构建成功后 Actions 尝试删除 `input.md` 和 `assets.zip`；
- 用户取消未启动任务时，取消接口立即尝试删除对应输入文件；
- 如果即时删除失败，记录仍会保留路径，后续过期清理可再次回收对象；
- 每日清理工作流删除过期任务对象并把状态更新为 `expired`；
- 单个任务清理失败不应阻塞后续任务；
- 清理日志不得输出密钥或签名 URL。

保留周期、收藏豁免和取消收藏后的过期时间重算应由数据库迁移与后端接口共同实现，不能只依赖前端状态。
...
运行 smoke test 前需要提供对应的 Supabase 和 GitHub Actions 环境变量。测试会创建临时用户与临时任务，验证上传、取消、幂等启动、工作流调度、状态推进、PDF 文件头、签名下载与最终清理流程。

## 常见错误

- `401`：前端会话失效，重新登录；
- 上传被 RLS 拒绝：检查迁移、Bucket 和对象路径；
- 取消返回 `409`：任务已经进入队列或状态发生变化；
