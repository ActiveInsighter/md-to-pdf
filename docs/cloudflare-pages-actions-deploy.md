...
后端、渲染器和纯文档改动不会触发 Pages。依赖新后端契约的前端必须先部署兼容的迁移与 Edge Functions，并通过手动 Supabase smoke，再合并前端。
...
→ 合并前端 PR，由 main push 自动部署 Pages
```
...
2. 优先打开截图 Artifact 中的 `ui-overview.png`，一次检查公开页面、登录后页面、桌面端和移动端；需要放大细节时再打开四张原始 PNG；
3. 临时测试用户已删除，截图 Artifact 只短期保留且不包含用户文件或密钥；
4. 登录后只能看到当前用户任务，切换账号不会显示上一账号缓存；
5. 创建任务、上传、取消会进入真实 `cancelled`，重复启动不会产生并行构建；
...
1. 在 Cloudflare Pages deployment 历史中选择上一已验证 production deployment 回滚，或通过 PR revert 前端 commit 后让 `deploy-pages.yml` 自动重新发布；
