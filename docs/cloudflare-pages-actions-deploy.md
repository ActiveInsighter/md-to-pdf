...
```text
在 GitHub Actions 为已合并、已验证的 main 版本手动运行 workflow_dispatch
→ 安装锁定的前端依赖
...
因此无需在 Pages 控制台配置框架、根目录、构建命令或输出目录，也不要把 GitHub 仓库导入 Cloudflare。
...
→ 手动运行 Pages 前端部署 workflow
```

如果前端需要新的后端行为，应拆成向后兼容的分阶段 PR：先合并并部署后端，完成函数验证和手动 smoke，再合并前端。`main` push 不会触发 smoke 或 Pages 部署，维护者必须确认后端已经就绪后再手动发布。
...
生产部署只通过 GitHub Actions 页面手动运行 `Deploy frontend to Cloudflare Pages`（`workflow_dispatch`）。合并或推送 `main`、修改 `frontend/**`、部署脚本或工作流都不会自动发布。

触发时选择已审查、已合并的 `main` 版本，并先确认数据库迁移和 Edge Functions 已部署、函数边界已验证、`Smoke test Supabase PDF service` 手动运行通过。后端、渲染器或文档改动不应无条件部署 Pages。
...
4. 创建任务、上传、取消会进入真实 `cancelled`，重复启动不会产生并行构建；
5. 前端状态以 Realtime 更新，断开 Realtime 时轮询能够兜底；
6. 完成任务通过短期签名 URL 下载，自动下载失败不会改变服务端 `completed`；
7. 浏览器网络与静态资源中不存在 `SUPABASE_SECRET_KEY`、`SUPABASE_SERVICE_ROLE_KEY` 或 `GITHUB_TOKEN`；
8. Cloudflare 没有接收或代理任何用户 Markdown、ZIP 与 PDF。
...
1. 在 Cloudflare Pages deployment 历史中选择上一已验证 production deployment 回滚，或通过 PR revert 前端 commit 后让 `deploy-pages.yml` 重新发布；
