...
const DESKTOP_GRID = 'grid-cols-[minmax(240px,1.05fr)_112px_minmax(390px,1.6fr)_150px_52px]'

export function JobList({ jobs, loading, emptyMessage = '暂无任务。' }: { jobs: PdfJob[]; loading?: boolean; emptyMessage?: string }) {
  if (loading) return <div className="space-y-3" role="status"><span className="sr-only">正在加载任务</span>{Array.from({ length: 5 }).map((_, index) => <Skeleton key={index} className="h-28 w-full" />)}</div>
    ...
    <>
      <div className="hidden overflow-x-auto rounded-xl border bg-card shadow-sm md:block">
        <div className="min-w-[980px]">
          <div className={`grid ${DESKTOP_GRID} items-center gap-5 border-b bg-muted/25 px-5 py-3 text-xs font-semibold text-muted-foreground`} role="row">
            <span>任务</span>
            <span>状态</span>
            <span>构建流程</span>
            <span>创建时间</span>
          ...
          <div className="divide-y">
            {jobs.map((job) => (
              <article key={job.id} className={`grid ${DESKTOP_GRID} items-center gap-5 px-5 py-5 transition-colors hover:bg-muted/20`}>
                ...
                <div className="whitespace-nowrap text-xs text-muted-foreground">
                  ...
                  <span className="mt-1 block">第 {job.attempt_count || 1} 次构建</span>
                </div>
        ...
        {jobs.map((job) => (
          <Card key={job.id} className="overflow-hidden">
            <CardContent className="space-y-4 p-4">
              ...
              <div className="overflow-x-auto pb-1"><JobProgress job={job} compact /></div>
              <div className="flex flex-wrap items-center justify-between gap-2 border-t pt-3 text-xs text-muted-foreground">
                <span>创建：{formatDateTime(job.created_at)}</span>
                <Button asChild variant="outline" size="sm"><Link to={`/jobs/${job.id}`}>查看详情</Link></Button>
              </div>
            </CardContent>
          </Card>
        ))}
